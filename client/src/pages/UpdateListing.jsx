...
export default function CreateListing() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
        ...
        if (data.success === false) {
            ...
            setFormData(data); // ✅ move this inside the else block
          }
        };
  ...
  const handleImageUpload = () => {
    ...
    const myWidget = window.cloudinary.openUploadWidget(
      {
        cloudName: 'dd54czsje',
        uploadPreset: 'mern estate',
        sources: ['local', 'url', 'camera'],
        multiple: true,
        maxFiles: 6 - formData.imageUrls.length,
        resourceType: 'image',
      ...
      (error, result) => {
        if (!error && result.event === 'success') {
          setFormData((prevState) => ({
            ...prevState,
            imageUrls: [...prevState.imageUrls, result.info.secure_url],
          }));
          setUploading(false);
        } else if (error) {
          console.error(error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success === false || !data._id) {
        return setError(data.message || 'Unknown error occurred');
      }
